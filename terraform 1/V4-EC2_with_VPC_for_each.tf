...
resource "aws_instance" "demo-server" {
   ...
   //  security_groups = ["demo-sg"]   we dont need this security_groups as VPC will be using its own security group
    vpc_security_group_ids = [aws_security_group.demo-sg.id]
    subnet_id = aws_subnet.dpp-public-subnet-01.id
    //To create multiple instances such as jenkins master and slave
